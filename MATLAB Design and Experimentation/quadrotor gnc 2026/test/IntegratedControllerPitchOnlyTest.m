classdef IntegratedControllerPitchOnlyTest < matlab.unittest.TestCase
    methods (Test)
        function testPitchDisturbanceNoCrossAxisLeak(testCase)
            dt = 0.01;
            numSteps = 500; % 5s, matching RollAngleCascadeTest's duration
            ...
            % Only pitch disturbed; roll/yaw/altitude start at rest/setpoint
            state.angle = [0; 30; 0];
            state.rate = [0; 0; 0];
            state.altitude = 0;
            ...
            testCase.verifyLessThan(abs(pitchHist(settleFrom:end)), 1);
