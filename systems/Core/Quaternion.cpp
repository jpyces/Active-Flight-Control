...
namespace gnc
{
    namespace
    {
        // Single shared degeneracy threshold -- a norm below this counts as
        // "effectively zero" everywhere in this file (normalize(),
        // isDegenerate(), inverse()). Previously inverse() compared norm()
        // *squared* against this same value, which silently loosened its
        // effective threshold on the norm itself by 1000x relative to the
        // other two methods -- fixed below by guarding on the norm before
        // squaring, same as normalize()/isDegenerate() do.
